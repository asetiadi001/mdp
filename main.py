...
class Main:
	...
	def ipWrite (self, delay, pc, ipq):
		...
		while stop_flag == 0:
			...
			if len(ipq) >0:
				msg = ipq.popleft()
				print "WiFi queue length after pop: " , len(btq)
				pc.write(msg)
	...
	def btWrite (self, delay, arduino, btq):
		stop_flag = 0
		while stop_flag == 0:
			time.sleep (delay)
			if len(btq) >0:
				msg = btq.popleft()
				print "IP queue length after pop: " , len(btq)
				arduino.write(msg)
	...
	def mainStart(self):
		...
		thread.start_new_thread (self.btWrite, (0.5, self.arduino, self.btq))
